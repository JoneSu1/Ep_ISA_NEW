...
# Internal imports
from Ep_ISA_NEW.modeling.predict import compute_predictions 
from Ep_ISA_NEW.utils import remove_if_exists, write_stream_csv, apply_threshold_filter
from Ep_ISA_NEW.scoring.null import derive_null_thresholds, sample_null_kmers
from Ep_ISA_NEW.scoring.utils_isa import load_pred_orig,ablate_motifs, region_str_to_seq
...
def run_single_isa(
    ...
    locs_df = (
        locs_df.sort_values("score", ascending=False)
        .drop_duplicates(subset=["chrom", "start", "end", "region"], keep="first")
